...
def train_function(model_args: ModelConfig, script_args: ScriptArguments, training_args: SFTConfig):
    ...
    dataset = load_dataset("csv", data_files=script_args.dataset_id_or_path)
    ...
    splitted_dataset = dataset["train"].train_test_split(test_size=0.1, seed=42)

    train_dataset = splitted_dataset["train"].map(
        formatting_prompts_func,
        batched=True,
        remove_columns=splitted_dataset["train"].column_names
    )
    eval_dataset = splitted_dataset["test"].map(
        formatting_prompts_func,
        batched=True,
    ...
    if trainer.accelerator.is_main_process:
        trainer.create_model_card({'tags': ['sft', 'qwen', 'duydc']})
